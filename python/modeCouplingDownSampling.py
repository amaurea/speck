...
def generateMCM(window,binLo,binHi,\
    ...
    if binningWeightMap ==None:
        ...
        print binningWeightMap.shape
        print powerMask.shape
    ...
    for ibin in xrange(len(binLo)):
        ...
        if binLo[ibin]>4000:
            divider=int(binLo[ibin]/1300-1)
            sPix=numpy.rint((npix-1)*numpy.random.rand(npix/divider))
            sPix=sPix.astype(int)
            sPix=numpy.sort(sPix)
            sPix=list(set(sPix))
            location=[location[0][sPix],location[1][sPix]]
            
            print "Numper of pix", len(location[0]),npix
            while len(location[0])<npix/(2*divider) :
                print "Too many doublon, Redraw a sample:"
                sPix=numpy.rint((npix-1)*numpy.random.rand(npix/divider))
                sPix=sPix.astype(int)
                sPix=numpy.sort(sPix)
                sPix=list(set(sPix))
